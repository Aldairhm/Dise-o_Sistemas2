const ROUTES = {
    ...
    toggleStatus: (id) =>
        `${window.APP_ROUTES.toggleStatus}/${id}/toggle-status`,
...
async function saveOrder() {
    ...
    const container =
        currentView === "table"
            ? "#categoriasBody .categoria-row"
            : "#cardsWrapper .categoria-card";
    const order = Array.from(document.querySelectorAll(container)).map(
        (el) => el.dataset.id,
    );
    ...
    try {
        ...
        if (!res.ok)
            throw new Error(data.message || "Error guardando el orden");

        Toast.fire({ icon: "success", title: "Orden guardado" });
    } catch (err) {
        console.error(err);
        Toast.fire({ icon: "error", title: "No se pudo guardar el orden." });
    }
}
...
function openCreateModal() {
    ...
    if (sub)
        sub.textContent = "Completa los campos para registrar una categoría";
...
function openEditModal(id, nombre, descripcion, color, icono) {
    ...
    const foundColor = (window.COLOR_PALETTE ?? []).find(
        (c) => c.hex === color,
    );
    selectColor(color, foundColor?.light ?? "#eff6ff");

    const foundIcon = (window.ICON_PALETTE ?? []).includes(icono)
        ? icono
        : "fa-tag";
...
function validarFormulario() {
    ...
    if (!descripcion) {
        showErrors({
            descripcion: ["La descripción de la categoría es obligatoria."],
        });
        isValid = false;
    }
...
document
    ...
    .addEventListener("submit", async function (e) {
        ...
        body.append(
            "descripcion",
            document.getElementById("inputDescripcion").value.trim(),
        );
        ...
        try {
            ...
            if (res.ok && data.success) {
                ...
                Toast.fire({ icon: "error", title: data.message ?? "Error inesperado." });
            }
        } catch {
            Toast.fire({ icon: "error", title: "No se pudo conectar al servidor." });
        } finally {
            btn.disabled = false;
...
async function toggleStatus(id, nombre, estadoActivo) {
    ...
    try {
        ...
        if (res.ok && data.success) {
            ...
            Toast.fire({ icon: "error", title: data.message ?? `No se pudo ${actionText}.` });
        }
    } catch {
        Toast.fire({ icon: "error", title: "No se pudo conectar al servidor." });
    }
...
function filterItems() {
    ...
    document.querySelectorAll(".categoria-row").forEach((r) => {
        r.style.display =
            !q || r.dataset.nombre.includes(q) || r.dataset.desc.includes(q)
                ? ""
                : "none";
    });
    document.querySelectorAll(".categoria-card").forEach((c) => {
        const m =
            !q || c.dataset.nombre.includes(q) || c.dataset.desc.includes(q);
...
function clearSearch() {
    ...
    document
        .querySelectorAll(".categoria-row, .categoria-card")
    ...
    try {
        ...
        if (data.stats) {
            ...
            document.getElementById("kpiActivas").textContent =
                data.stats.activas;
            document.getElementById("kpiInactivos").textContent =
                data.stats.inactivos;
        }
